...
geofcm <- pdata_fcm %>%
  ...
  mutate(
    ...
    scb_region_map = case_when(
      scb_region == "01" ~ "Stockholm",
      scb_region == "03" ~ "Uppsala",
      scb_region == "04" ~ "Södermanland",
      scb_region == "05" ~ "Östergötland",
      scb_region == "06" ~ "Jönköping",
      scb_region == "07" ~ "Kronoberg",
      scb_region == "08" ~ "Kalmar",
      scb_region == "09" ~ "Gotland",
      scb_region == "10" ~ "Blekinge",
      scb_region == "12" ~ "Skåne",
      scb_region == "13" ~ "Halland",
      scb_region == "14" ~ "Västra Götaland",
      scb_region == "17" ~ "Värmland",
      scb_region == "18" ~ "Orebro",
      scb_region == "19" ~ "Västmanland",
      scb_region == "20" ~ "Dalarna",
      scb_region == "21" ~ "Gävleborg",
      scb_region == "22" ~ "Västernorrland",
      scb_region == "23" ~ "Jämtland",
      scb_region == "24" ~ "Västerbotten",
      scb_region == "25" ~ "Norrbotten"
    ),
...
mykable(geofcm %>% select(scb_region_txt, np),
  col.names = c("Region", "N FCM uses/N Total (%)"),
...
mapcols <- rev(c(
  ...
  grDevices::rgb(189, 171, 179, maxColorValue = 255)
))
