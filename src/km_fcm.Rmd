...
kmfunc <- function(time, event, eventname, yposplus = rep(0, 3)) {
  ...
  ## logrank
  sdu <- survdiff(formula(paste0("Surv(", time, ",", event, "=='Yes') ~ shf_ferrocarboxymaltosis")),
    data = pdata_fcm
  )
  ...
  ## logrank
  sdm <- survdiff(formula(paste0("Surv(", time, ",", event, "=='Yes') ~ shf_ferrocarboxymaltosis")),
    data = matchp_fcm
  )
  ...
  text(20, 0.7, paste0("Log-rank all p = ", pu), pos = 4)
  text(20, 0.65, paste0("Log-rank matched p = ", pm), pos = 4)
}
```
