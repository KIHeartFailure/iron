...
kmfunc <- function(time, event, eventname, yposplus = rep(0, 3)) {
  ...
  ylabs <- bind_cols(
    ypos = c(yposm + yposplus[1:2], yposu[1] + yposplus[3]),
...
kmfunc(
  ...
  yposplus = c(0.025, 0, -0.023) # matched no, matched yes, overall no
)
```
...
kmfunc(
  ...
  yposplus = c(0, 0.02, 0.00) # matched no, matched yes, overall no
)
```
