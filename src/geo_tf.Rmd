...
geotf <- pdata_tf %>%
  ...
  ungroup() %>%
  pivot_wider(names_from = shf_tf2, values_from = c(n, percent)) %>%
  mutate(
...
mykable(geotf %>% select(scb_region_txt, np),
  col.names = c("Region", "N with transferrin and/or ferritin measure/N Total (%)"),
  caption = "Geographical distribution of transferrin/ferritin measure in Sweden",
  scale_down = F
)
```

```{r geotfmap, fig.cap="Geographical distribution of transferrin/ferritin measure in Sweden", cache=cacheon, dependson="geotf"}
