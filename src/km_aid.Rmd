...
kmfunc <- function(time, event, eventname, yposplus = rep(0, 4)) {
  ...
  ## logrank
  sd <- survdiff(formula(paste0("Surv(", time, ",", event, "=='Yes') ~ shf_aid")),
    data = pdata
  )
  ...
  text(20, 0.6, paste0("Log-rank p = ", p), pos = 4)
}
```
...
kmfunc(
  ...
  yposplus = c(0, +0.01, 0, 0) # lowest, middle, highest
)
```
...
kmfunc(
  ...
  yposplus = c(-0.015, +0.015, -0.005, 0.005) # lowest, middle, highest
)
```
...
kmfunc(
  ...
  yposplus = c(-0.01, 0, 0, 0) # lowest, middle, highest
)
```
